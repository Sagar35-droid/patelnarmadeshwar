      ...
      <section className="bg-[#1A1A1A] text-white py-14 border-b-2 border-[#C5A059]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
          ...
          <h1 className="text-3xl sm:text-5xl font-serif font-bold tracking-tight text-white">
            Narmadeshwar Shivling Collection (नर्मदेश्वर शिवलिंग)
          </h1>
          <p className="text-[#F3EFE9] text-xs sm:text-sm max-w-3xl mx-auto leading-relaxed font-light">
            Explore 100% authentic, naturally formed Narmadeshwar Shivlings directly from the sacred Narmada River in Bakawan, Madhya Pradesh. Available in sizes from 1.5 inches to 1 foot with matching Jaladhari, natural Janeyu, and Tilak marks for your home temple.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-2 pt-2 text-[11px] text-stone-300">
            <span className="bg-[#2D2D2D] px-3 py-1 rounded-full border border-stone-700">100% Natural River Stone</span>
            <span className="bg-[#2D2D2D] px-3 py-1 rounded-full border border-stone-700">Sourced from Bakawan (MP)</span>
            <span className="bg-[#2D2D2D] px-3 py-1 rounded-full border border-stone-700">Wooden Box Safe Packaging</span>
            <span className="bg-[#2D2D2D] px-3 py-1 rounded-full border border-stone-700">Pan-India Direct Delivery</span>
          </div>
        </div>
      </section>

      {/* Product Cards Grid - All 38 Existing Products */}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          {products.map((product) => (
            <ProductCard key={product.id} product={product} hideTitle={false} />
          ))}
        </div>
      </section>
